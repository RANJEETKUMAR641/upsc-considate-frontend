...
function CandidateLogin() {
  ...
  const handleOnFormChange = useCallback((values) => {
    if (values?.registrationid?.length > 16) {
      methods.setError('registrationid', { message: 'invalid regid' })
    }

    setType(get(values, 'login_type.value', get(values, 'login_type')))
  }, [])
  ...
  return (
    <LoginNewStyle>
      <Suspense fallback={<OverlayLoader open className="" />}>
        <div>
          <Container className="p-sm-0 pe-sm-2">
            <Row>
              <Col sm={{ size: 4, order: 2 }} className="p-sm-0">
                <div className="mb-3">
              ...
              <Col sm={{ size: 4, order: 1 }} className="mb-3">
                {/* <Steps /> */}
                <CandOtrFeature />
              </Col>
